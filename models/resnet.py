...
'''
这段代码定义了一个基于ResNet（残差网络）的变体，称为Multi_ResNet，
用于深度学习任务中的图像分类。代码中实现了两种基本构建块：BasicBlock和BottleneckBlock，
并在Multi_ResNet类中使用这些构建块来构建不同深度的网络。此外，还引入了一种特殊的分支结构branchBottleNeck，
旨在从不同深度的特征图中提取信息。这个网络设计支持多输出，可以用于知识蒸馏（Knowledge Distillation）等场景，
其中中间特征也被用于训练过程。下面是代码的详细解释
'''
...
class BasicBlock(nn.Module):
    '''
    BasicBlock：是一个包含两个3x3卷积层的简单残差块。它用于构建较浅的网络（如ResNet-18）。
    '''
    expansion = 1
...
class BottleneckBlock(nn.Module):
    '''
    BottleneckBlock：包含了一个1x1卷积（降维），一个3x3卷积（处理），和一个1x1卷积（升维）的结构。
    这种设计旨在减少计算量的同时保持网络性能，通常用于构建较深的网络（如ResNet-50）
    '''
    expansion = 4
    def __init__(self, inplanes, planes, stride=1, downsample=None):
