'''如果不采用知识蒸馏的形式来训练模型，就需要像本文件一样对源文件进行修改，另外需要将‘train_nodistiller.py’文件改为‘train.py’文件:
        1、损失值只考虑真实标签与最后一层logits做的损失：注释其他两个损失值的计算
        2、反向传播的时候只传播保留的损失值,不传播原来文件的总损失
        3、如果不想保留每个epoch的值，也要像本文件那样修改'''
import torch
...
def run_training(args):
    ...
    for current_epoch in range(args.start_epoch, args.epoch):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            # total_loss.backward() # 反向传播的总损失
            loss.backward() # 反向传播的最后一类的损失
            optimizer.step()
        ...
        # shutil.copyfile(checkpoint_path, os.path.join(args.save_path, 'checkpoint_latest.pth.tar')) # 使用shutil.copyfile方法将当前保存的检查点复制一份到相同目录下，并命名为model_best.path.tar
